...
const guncelleUI=(veri)=>{
    ...
    const iconSrc=`img/icons/${havaDurumu.WeatherIcon}.svg`;  /* hava durumu iconumzu ekrandda gosterdık */
    ...
    let zamanSrc=havaDurumu.IsDayTime?'img/day.svg':'img/night.svg'; // if els  yapısının kısaltılmış hali
    
    zamanResmi.setAttribute('src',zamanSrc);

    if(kart.classList.contains('d-none')){  //kart kısmını aktif etmek için kulandık 
...
form.addEventListener('submit',(e)=>{
    ...
    sehirGuncelle(sehir)
        .then(veri=>{
            guncelleUI(veri);
            console.log(veri); //log ekranında göstermesi için 
        });


    form.reset();

    localStorage.setItem('sehir',sehir);  // şehir bilgisi tutmasını saglaycak
});
...
if(localStorage.getItem('sehir')){   // icerisinde bilgi varmı eyer var ise 
